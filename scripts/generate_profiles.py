...
from models import Assistant, Transcriber, Model, Message, Voice
from api_functions.openai_api_functions import generate_llm_response
from typing import List

def generate_assistant_profiles(num_profiles: int, 
                                voice_agent_system_prompt: str,
                                voice_agent_type: str) -> List[Assistant]:
    """
    Generates multiple new 'test assistant' objects in a single LLM call.
    The LLM is instructed to return a JSON array of length num_profiles.
    
    Example JSON format expected:
    [
      {
        "name": "...",
        "content": "...",
    ...
    Args:
        num_profiles: How many new assistants to generate in one LLM response.
        voice_agent_system_prompt: Context describing the voice agent that is being tested (system prompt or summary).

    Returns:
        A list of newly created Assistant objects.
    """

    # Prompt for generating assistant profiles
    prompt = f"""
    You are given the following context about a {voice_agent_type} voice agent: {voice_agent_system_prompt}. 
    Based on this context, generate {num_profiles} distinct profiles with different personalities and issues
    for new voice agents who will call the main voice agent.

    **Requirements**:
    - For each test assistant, provide:
      1. "name": a name for the test assistant.
      2. "content": effectively the system prompt for this new assistant, 
         describing its personality traits and motivations in a few sentences.
      3. "firstMessage": the first thing this new assistant will say.
      4. "maxDurationSeconds": an integer <= 30.

    **Output Format**:
    Return valid JSON. Specifically, return an array with {num_profiles} elements. 
    ...
    Example of the shape (with fewer items) might be:
    [
      {{
        "name": "Assistant One",
        "content": "System prompt for assistant one...",
        "firstMessage": "Hello, I'm assistant one!",
        "maxDurationSeconds": 20
      }},
      {{
        "name": "Assistant Two",
        "content": "System prompt for assistant two...",
        "firstMessage": "Hello, I'm assistant two!",
        "maxDurationSeconds": 30
      }}
    ]

    Now output your JSON array with exactly {num_profiles} objects. Make sure it is ONLY a valid JSON array. 
    The response should be parseable by json.loads() in Python. Do not include ```json at the beginning or end of your response.
    """
    # Single LLM call to generate all new assistants
    llm_response = generate_llm_response(prompt, model="gpt-4o-mini")

    # Parse the entire response as JSON
    try:
        assistants_data = json.loads(llm_response)
    except json.JSONDecodeError as e:
        raise ValueError(
            f"Failed to parse LLM output as valid JSON.\nError: {e}\nLLM Response:\n{llm_response}"
        )

    # Make sure we got a list
    if not isinstance(assistants_data, list):
        raise ValueError(
            f"The LLM response must be a JSON array/list, but we got: {type(assistants_data)}"
        )

    # Ensure the list has the requested number of items
    if len(assistants_data) != num_profiles:
        raise ValueError(
            f"Expected {num_profiles} items in the JSON array, but got {len(assistants_data)}.\n"
            f"Full JSON: {assistants_data}"
        )

    # Build each Assistant from the list
    new_assistants = []
    for idx, assistant_data in enumerate(assistants_data, start=1):
        # Validate required fields
        required_fields = ["name", "content", "firstMessage", "maxDurationSeconds"]
        for field in required_fields:
            if field not in assistant_data:
                raise ValueError(
                    f"Item #{idx} is missing required field '{field}': {assistant_data}"
                )

        # Check maxDurationSeconds <= 30
        if assistant_data["maxDurationSeconds"] > 30:
            raise ValueError(
                f"Item #{idx} has maxDurationSeconds over 30, which violates the requirement. "
                f"Got: {assistant_data['maxDurationSeconds']}"
            )

        # Construct the Assistant object
        new_assistant = create_assistant_with_default_values(
            name=assistant_data["name"],
            first_message=assistant_data["firstMessage"],
            system_prompt=assistant_data["content"],
            max_duration_seconds=assistant_data["maxDurationSeconds"]
        )
        new_assistants.append(new_assistant)

    return new_assistants

def create_assistant_with_default_values(name: str, 
                                         first_message: str, 
                                         system_prompt: str, 
                                         max_duration_seconds: int) -> Assistant:
    return Assistant(
                name=name,
...
def retrieve_voice_agent_type():
    """
    Retrieves the voice agent type. Currently hardcoded, but will be retrieved from the voice agent in the future.
    """
    return "appointment scheduling"

def retrieve_system_prompt():
    """
    Retrieves the system prompt for the voice agent. Currently hardcoded, but will be retrieved from the voice agent in the future.
    """
    return """# Appointment Scheduling Agent Prompt
